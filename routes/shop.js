...
function buildProductQuery({ shopId, category, q, minPrice, maxPrice }) {
    let query = `
        SELECT p.*,
...
router.get('/', async (req, res) => {
    ...
    const { query, params } = buildProductQuery({ shopId, category, q, minPrice, maxPrice });
    ...
    const { rows: categories } = await pool.query('SELECT DISTINCT category FROM products WHERE category IS NOT NULL AND shop_id = $1', [shopId]);
    ...
    const { rows: categoryThumbRows } = await pool.query(`
        ...
        FROM products
        WHERE category IS NOT NULL AND image_url IS NOT NULL AND shop_id = $1
    ...
    const { rows: bestSellerRows } = await pool.query(`
        ...
        WHERE o.status != 'cancelled' AND o.shop_id = $1
    ...
    if (bestSellerIds.length > 0) {
        const { rows } = await pool.query(
            `SELECT p.*, ROUND(p.price * (1 - p.discount_percent / 100.0)) AS effective_price
             FROM products p
             WHERE p.id = ANY($1::int[]) AND p.shop_id = $2
             ORDER BY array_position($1::int[], p.id)`,
            [bestSellerIds, shopId]
        );
        bestSellers = rows;
    }
...
router.post('/cart/add', async (req, res) => {
    ...
    if (vid) {
        ...
        const { rows } = await pool.query('SELECT id FROM products WHERE id = $1 AND shop_id = $2', [pid, req.shop.id]);
        if (rows.length === 0) {
            if (req.get('X-Requested-With') === 'fetch') return res.status(400).json({ ok: false, message: 'পণ্য খুঁজে পাওয়া যায়নি।' });
            return res.redirect('back');
        }
    }
    ...
    if (req.get('X-Requested-With') === 'fetch') {
        const { items, subtotal } = await cartService.getCartItems(req.session, req.shop.id);
...
router.post('/coupon/apply', couponLimiter, async (req, res) => {
    const { subtotal } = await cartService.getCartItems(req.session, req.shop.id);
    const result = await validateCoupon(req.body.code, subtotal, req.shop.id);
    if (!result.ok) {
...
router.get('/checkout', async (req, res) => {
    ...
    if (req.session.couponCode) {
        const result = await validateCoupon(req.session.couponCode, subtotal, req.shop.id);
        if (result.ok) {
...
router.post('/checkout', checkoutLimiter, async (req, res) => {
    ...
    const deliveryCharge = cartService.calculateDeliveryCharge(city, req.shop);
    ...
    try {
        ...
        const orderResult = await client.query(
            `INSERT INTO orders (customer_name, phone, address, city, total, payment_method, customer_id, subtotal, discount_amount, delivery_charge, coupon_code, shop_id)
             VALUES ($1, $2, $3, $4, $5, 'cod', $6, $7, $8, $9, $10, $11) RETURNING id`,
            [customerName, phone, address, city || '', total, req.session.customerId || null, subtotal, discountAmount, deliveryCharge, couponCode, req.shop.id]
        );
        ...
        const orderForNotify = { id: orderId, customer_name: customerName, phone, address, city: city || '', total, subtotal, discount_amount: discountAmount, delivery_charge: deliveryCharge, coupon_code: couponCode };
...
router.get('/order/:id/invoice', async (req, res) => {
    ...
    const { rows: orderRows } = await pool.query('SELECT * FROM orders WHERE id = $1 AND phone = $2 AND shop_id = $3', [req.params.id, phone, req.shop.id]);
...
async function lookupOrdersByPhone(phone, shopId) {
    const { rows: orders } = await pool.query(
        'SELECT * FROM orders WHERE phone = $1 AND shop_id = $2 ORDER BY created_at DESC',
        [phone, shopId]
    );
...
router.get('/track-order', async (req, res) => {
    ...
    const { orders, orderItemsById } = await lookupOrdersByPhone(phone, req.shop.id);
