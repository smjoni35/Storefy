...
router.get('/register', (req, res) => {
    if (req.session.customerId && req.session.customerShopId === req.shop.id) return res.redirect(`${req.shopPath}/account/dashboard`);
...
router.post('/register', customerRegisterLimiter, async (req, res) => {
    ...
    try {
        const existing = await pool.query('SELECT id FROM customers WHERE phone = $1 AND shop_id = $2', [cleanPhone, req.shop.id]);
        ...
        const { rows } = await pool.query(
            'INSERT INTO customers (name, phone, email, password_hash, shop_id) VALUES ($1, $2, $3, $4, $5) RETURNING id, name',
            [name.trim(), cleanPhone, (email || '').trim() || null, hash, req.shop.id]
        );

        req.session.customerId = rows[0].id;
        req.session.customerName = rows[0].name;
        req.session.customerShopId = req.shop.id;
        res.redirect(`${req.shopPath}/account/dashboard`);
    } catch (err) {
...
router.post('/login', customerLoginLimiter, async (req, res) => {
    ...
    const { rows } = await pool.query('SELECT * FROM customers WHERE phone = $1 AND shop_id = $2', [cleanPhone, req.shop.id]);
    if (rows.length === 0) {
    ...
    const safeNext = next && next.startsWith('/') && !next.startsWith('//') ? next : `${req.shopPath}/account/dashboard`;
...
router.get('/dashboard', requireCustomer, async (req, res) => {
    ...
    const { rows: orders } = await pool.query(
        'SELECT * FROM orders WHERE customer_id = $1 AND shop_id = $2 ORDER BY created_at DESC',
        [customerId, req.shop.id]
    );
...
router.get('/orders/:id/invoice', requireCustomer, async (req, res) => {
    const { rows: orderRows } = await pool.query(
        'SELECT * FROM orders WHERE id = $1 AND customer_id = $2 AND shop_id = $3',
        [req.params.id, req.session.customerId, req.shop.id]
    );
