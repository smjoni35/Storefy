...
async function getCartItems(session, shopId) {
    ...
    // shop_id filter here is what stops a cart line for shop A's product id
    // from resolving against shop B's catalog if the two ever collide.
    const { rows: products } = await pool.query('SELECT * FROM products WHERE id = ANY($1::int[]) AND shop_id = $2', [productIds, shopId]);
    const productsById = {};
...
function calculateDeliveryCharge(city, shop) {
    const insideDhaka = /dhaka|ঢাকা/i.test((city || '').trim());
    // Each shop sets its own rates (shops.delivery_charge_dhaka/outside);
    // 0 in the DB falls back to the old env vars so the original default
    // shop's behavior is unchanged until someone edits it.
    const shopInside = shop && parseFloat(shop.delivery_charge_dhaka);
    const shopOutside = shop && parseFloat(shop.delivery_charge_outside);
    const inside = shopInside || parseFloat(process.env.DELIVERY_CHARGE_DHAKA) || 70;
    const outside = shopOutside || parseFloat(process.env.DELIVERY_CHARGE_OUTSIDE) || 130;
