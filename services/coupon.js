...
async function validateCoupon(code, subtotal, shopId) {
    if (!code || !code.trim()) {
        return { ok: false, message: 'কুপন কোড দিন।' };
    }

    const { rows } = await pool.query('SELECT * FROM coupons WHERE UPPER(code) = UPPER($1) AND shop_id = $2', [code.trim(), shopId]);
    if (rows.length === 0) {
